...
public class ListenerService {
    ...
    <T extends Type> void createMultiple(Configuration configuration, Class<T> type, ListenerServiceRequest request) {
        // Find a listener for the desired Type and event name
        Class<? extends Listener> listener = listenerRepository.findListener(type, request.getListenType());

        // Parse the incoming object data as a List of the desired Type
        List<T> values = valueParser.asList(request.getValueForListening().getObjectData(), type);

        // Create the listener using the defined handler in the service, with the parsed EngineValue
        injector.getInstance(listener)
                .createMultiple(configuration, values);
    }

    @SuppressWarnings("unchecked")
    <T extends Type> void createSingle(Configuration configuration, Class<T> type, ListenerServiceRequest request) {
        // Find a listener for the desired Type and event name
        Class<? extends Listener> listener = listenerRepository.findListener(type, request.getListenType());

        // Parse the incoming object data as a single Object of the desired Type
        T value = valueParser.asObject(request.getValueForListening().getObjectData(), type);
